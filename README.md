...
DesignGuardian AI implements a **Model Context Protocol (MCP)** server, enabling developers and judges to query the design constitution directly within VS Code's Copilot Chat or other MCP-compatible hosts.

### 1. Launch the MCP Server
Start the server in MCP stdio transport mode:
```bash
npm run start:mcp
```

### 2. Connect to GitHub Copilot in VS Code
Add DesignGuardian to your Copilot Configuration (`~/.config/github-copilot/apps.json` or the Copilot settings panel):
```json
{
  "mcpServers": {
    "designguardian-ai": {
      "command": "node",
      "args": ["c:/Users/acer/OneDrive/Desktop/Microsoft Hackathon/server/dist/mcp-server.js"]
    }
  }
}
```
*(Adjust the absolute path to point to your local build).*

Once connected, ask Copilot directly in the chat panel:
*   `@copilot /designguardian audit the active file`
*   `@copilot /designguardian explain why my button does not meet the guidelines`
*   `@copilot /designguardian get the design DNA of my card`

---

## 🧬 Multi-Step AI Reasoning Pipeline (Rubric: Reasoning & Multi-step Thinking)

DesignGuardian AI uses a 4-phase grounded reasoning pipeline to prevent hallucinations and ensure that all AI critiques are anchored directly in your design constitution:

```mermaid
graph TD
    Code["💻 Developer Code"] --> Static["🔍 1. Static Scan (Instant Token Check)"]
    Static --> Retrieve["⚡ 2. RAG Retrieval (Microsoft Foundry IQ)"]
    Retrieve --> Gemini["🧠 3. Gemini Reasoning (gemini-2.5-flash)"]
    Gemini --> Reasoning["💡 4. Reasoning & UX Impact Synthesis"]
    Reasoning --> Refactor["⚙️ 5. Automated Refactoring & Diff Output"]

    %% Styles
    classDef step fill:#181825,stroke:#89b4fa,stroke-width:2px,color:#cdd6f4;
    class Code,Static,Retrieve,Gemini,Reasoning,Refactor step;
```

1.  **Static Scan**: The local analyzer flags raw tokens (e.g., hardcoded hex colors, unaligned spacing, missing alt tags).
2.  **Semantic Retrieval**: Microsoft Foundry IQ performs semantic lookup against our local/cloud knowledge base (`skills/*.md`) to find matching rules.
3.  **Gemini Reasoning**: The code, violations, and retrieved design documents are passed to Google Gemini to formulate the design coach critique and reasoning chain.
4.  **Refactoring Output**: The refactoring engine outputs the AST-safe JSX code replacements and applies them directly to the active editor.
...
*   **GitHub Copilot & MCP Integration**: Integrates directly with GitHub Copilot Chat in VS Code via the Model Context Protocol (MCP) server, offering an outstanding developer workflow.
...
*   **Reasoning & Multi-step Thinking**: Employs a multi-phase audit and auto-fix pipeline, explaining the design logic and expected UX impact for every single suggestion.
